...
namespace ZGZY.WebUI.fabview.ashx.BasicData
{
    /// <summary>
    /// CodeList 的摘要说明
    /// </summary>
        ...
        void InsertLines(HttpContext context)
        {
            ...
            if (count > 0)
            {
                sql = @"update hcode_line 
                            set code_desc='{0}',code1='{1}', code2='{2}', code3='{3}', code4='{4}', 
                                code5='{5}' ,last_updated_date=getdate(),last_updated_by='{6}'
                                where code_cate = '{7}' and code_value='{8}';";
                sql = string.Format(sql, strCodeDesc, strCode1, strCode2, strCode3, strCode4, strCode5, strUser, strCodeCate, strCodeValue);
            }
            else
            {
